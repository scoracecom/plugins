...
> **当前状态：`0.1.1` 公开试用已发布。** 真实双平台 runtime lock、projection、payload、公开下载与安装均已完成最终校验。Windows 正式入口为原生 `tools/scorace-runtime.cmd`。不要降低或绕过 PowerShell ExecutionPolicy，也不要关闭任何系统保护。
...
当前 `0.1.1` release 已作为公开试用发布。安装后请开启新的 Codex 会话，以加载 `scorace-learning` Skill。卸载使用：
...
- 程序：`scorace` `0.1.1`
- 发布标签：[`scorace-v0.1.1`](https://github.com/scoracecom/plugins/releases/tag/scorace-v0.1.1)
