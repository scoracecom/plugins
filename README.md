...
> **当前状态：暂停公开下载。** `0.1.0 release currently paused/not publicly downloadable`：Parallels Windows 11 ARM64 的正常用户在 `ExecutionPolicy=Restricted` 下，会在 `.ps1` helper 逻辑启动前触发 `PSSecurityException`。请不要降低或绕过 `ExecutionPolicy`，也不要关闭任何系统保护。macOS artifact 已验证，但整个双平台 trial 尚未放行。
...
安装命令仅保留候选仓格式；上述 release 暂停期间，不应视为可用的公开下载入口。安装后请开启新的 Codex 会话，以加载 `scorace-learning` Skill。卸载使用：
...
- [macOS arm64 ZIP](https://github.com/scoracecom/plugins/releases/download/scorace-v0.1.0/scorace-0.1.0-darwin-arm64.zip)（当前不可用）：Developer ID 签名及公证已 Accepted。
- [Windows x64 ZIP](https://github.com/scoracecom/plugins/releases/download/scorace-v0.1.0/scorace-0.1.0-windows-x64.zip)（当前不可用）：hosted x64 CI 已通过；Parallels consumer validation pending。
