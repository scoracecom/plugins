...
这是 ScorAce 的宿主 Plugin 候选 `0.8.0-candidate.20260922.3`：包含 Skill、方法资源和发行清单，不携带学习核心源码、旧 binary、SEA 或平台 runtime helper。宿主负责会话、文件、授权、命令执行和呈现；Skill 只提供学习工作指引。受管学习操作统一通过 npm 包 `@scorace/cli` 执行；本候选的实际验证版本为 `0.1.4`，Plugin 的官方 `.codex-plugin/plugin.json` 不承载 CLI 私有字段。
...
`learning-release.json` 固定记录 `cli_package: "@scorace/cli"` 与 `cli_protocol: 5`，以及 Plugin/API/Core 版本和公开文件摘要；这些字段不复制到官方宿主 manifest。npm 是 CLI 的发行机制，Plugin 的安装、启用、更新和卸载仍沿用宿主标准机制。
